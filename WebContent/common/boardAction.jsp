...
<%	
	...
	if("write".equals(mode)) {
		...
		write_count = boardService.DataWrite(dataVO);
		...
		/* -------------- 첨부파일 검증 및 등록 - S -------------- */		
		if(write_count > 0) { //글등록 성공시 첨부파일등록 검증
			...
			if(!upload_flag) { //파일업로드 불허상태라면 파일삭제
				...
				if(fileList != null && fileList.size() > 0) {
					...
					for(FileVO fileVO : fileList) {
						file_write_count = fileService.FileWrite(fileVO);
						
						if(file_write_count == 0) { //등록실패
							%>
								<script type="text/javascript">
